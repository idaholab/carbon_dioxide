...
extern "C" void  __stdcall VU_HP_L_INI_CO2(double h, double p, double& v, double& u);
extern "C" void  __stdcall VU_HP_G_INI_CO2(double h, double p, double& v, double& u);
//
//scaling functions
...
SBTLAPI int __stdcall PH_FLASH_L(double p, double h, double& v, double& u) throw()
{
    ...
    if(fabs(p-pc)<0.1 && fabs(h-hc)<0.2) {
        v=vc;
        u=uc;
        return I_OK;
    }
